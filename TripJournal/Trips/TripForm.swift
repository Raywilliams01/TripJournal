...
struct TripForm: View {
    ...
    init(mode: Mode, updateHandler: @escaping () -> Void) {
        ...
        case let .edit(trip):
            ...
            _startDate = .init(initialValue: trip.start_date)
            _endDate = .init(initialValue: trip.end_date)
        }
    }
    ...
    private func addTrip() async {
        ...
        do {
            try validateForm()
            let request = TripCreate(name: name, start_date: startDate, end_date: endDate)
    ...
    private func editTrip(withId id: Trip.ID) async {
        ...
        do {
            try validateForm()
            let request = TripUpdate(name: name, start_date: startDate, end_date: endDate)
